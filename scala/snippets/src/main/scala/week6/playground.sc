...
//object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.groupBy(x => x.toLower).mapValues(str => str.length).toList.sorted

  wordOccurrences("Example": Word)
  ...
  sentenceOccurrences(List("Exa", "mple"):Sentence)
  sentenceOccurrences(List():Sentence)
  ...
  dictionaryByOccurrences.take(3)

  /** Returns all the anagrams of a given word. */
  def wordAnagrams(word: Word): List[Word] =
    (dictionaryByOccurrences withDefaultValue List())(wordOccurrences(word))

  wordAnagrams("scala": Word)
  wordAnagrams("ate": Word)
  wordAnagrams("non_existing_word": Word)
...
def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
  ...
  case head :: tail =>
    ...
    (for {
      count <- 1 to head._2
      s <- subsets
    } yield (head._1, count) :: s).toList ++ subsets

}


val comb = combinations(List(('a', 2), ('b', 2)): Occurrences)

comb == List(List(('a',1), ('b',1)), List(('a',1), ('b',2)),
  List(('a',1)), List(('a',2), ('b',1)), List(('a',2), ('b',2)),
