...
f(100)
  ...
  val t1 = new Tweet("user1", "1", 5)
  val t2 = new Tweet("user2", "2", 10)
  val t3 = new Tweet("user3", "3", 15)
  val t4 = new Tweet("user4", "4", 20)
  val t5 = new Tweet("user5", "5", 25)

  var empty: TweetSet = new Empty()

  new Tweet("user1", "text1", 5)

  val tweetset: TweetSet = new NonEmpty(t1, new Empty(), new Empty()).incl(t2).
    incl(t3).incl(t4).incl(t5)

  val tweetset1: TweetSet = new NonEmpty(t3, new Empty(), new Empty()).incl(t2)
  val tweetset2: TweetSet = new NonEmpty(t3, new Empty(), new Empty()).incl(t4)

  tweetset1.contains(t1)
  tweetset1.filter(tweet => tweet.retweets > 7)

  println("union of " + tweetset1 + " and " + tweetset2 +
  " equals " + tweetset1.union(tweetset2))

tweetset.mostRetweeted
empty.mostRetweeted

tweetset.descendingByRetweet
//}
